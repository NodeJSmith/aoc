...
def get_previous_number(numbers_lists: list[list[int]]):
    numbers_lists[-1].insert(0, 0)
    numbers_lists = list(reversed(numbers_lists))
    for i in range(len(numbers_lists) - 1):
        num_list = numbers_lists[i]
        next_num_list = numbers_lists[i + 1]
        num_a = num_list[0]
        num_b = next_num_list[0]

        diff = num_b - num_a
        print(f"{num_b:<4} - {num_a:<4} = {diff:<4}")
        next_num_list.insert(0, diff)

    return numbers_lists[-1][0]
...
for line in lines:
    ...
    next_num = get_previous_number(output)
